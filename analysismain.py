...
class TrafficDataAnalyzer:
    def __init__(self, base_path, analysis_outputs_root="analysis outputs"):
        self.base_path = base_path
        # Create the root analysis outputs directory if it doesn't exist
        self.analysis_outputs_root = os.path.join(base_path, analysis_outputs_root)
        os.makedirs(self.analysis_outputs_root, exist_ok=True)
        
        # Generate unique output directory name with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.output_dir = os.path.join(self.analysis_outputs_root, f"analysis_{timestamp}")
...
if __name__ == "__main__":
    try:
        ...
        analyzer = TrafficDataAnalyzer(base_path)
        ...
        print(f"\nAll analysis outputs saved to: {analyzer.output_dir}")
    except Exception as e:
        print(f"Error: {str(e)}")
